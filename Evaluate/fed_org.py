...
def load_models(name="microsoft/DialoGPT-large"):
  tokenizer = AutoTokenizer.from_pretrained(name, mirror='tuna')
  model = AutoModelWithLMHead.from_pretrained(name, mirror='tuna')
...
#     device = 'cuda'
  ...
  loss = score_batch(texts, tokenizer, model)
  idx = 0
  for metric, utts in turn_level_utts.items():
    pos, neg = utts["positive"], utts['negative']
    if len(pos) > 0:
